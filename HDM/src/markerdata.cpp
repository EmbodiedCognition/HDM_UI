    ...
    QObject(parent)
{
  ...
  for (int i = 0; i < 41; i++) {
      ...
      labels.push_back(fulllabel.substr(fulllabel.find(":")+1));
  }
  loadSetting();
}
...
void MarkerData::reloadMarkerfile(const char* filename){
    this->world=world;
    this->space=space;
...
void MarkerData::reloadCMarkerfile(const char* filename){

    this->world=world;
    this->space=space;
    data.loadFile(filename);
    std::cout << "[markerdata] Custom Marker file loaded...";
    dJointGroupEmpty(joint_group);
    loadSetting();

}

void MarkerData::loadSetting(){
