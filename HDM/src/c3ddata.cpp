...
*/
bool C3dFile::loadFile_c3d(const char* filename)
{
        ...
        parameters.loadParameters(c3dFile,header);
         ...
         //check if the loaded C3D File is correct or not
         std::map <string,C3dGroup*>::iterator it;
         it = parameters.groups.find("ANALOG");
         if (it == parameters.groups.end()){
             std::cout << "[c3ddata] .c3d file is not correct: parameters.groups[\"ANALOG\"] not found!" << std::endl;
             return false;
         }

         it = parameters.groups.find("POINT");
         if (it == parameters.groups.end()){
             std::cout << "[c3ddata] .c3d file is not correct: parameters.groups[\"POINT\"] not found!" << std::endl;
             return false;
         }

         it = parameters.groups.find("TRIAL");
         if (it == parameters.groups.end()){
             std::cout << "[c3ddata] .c3d file is not correct: parameters.groups[\"TRIAL\"] not found!" << std::endl;
             return false;
         }
...
/////////load csv file///////////
void C3dData::loadData_csv(const char *filename)
{
    ...
    if(!ip.is_open()) std::cout << "ERROR: File Open" << '\n';
      ...
      pointsPerFrame = 50;
      std::cout << "pointsPerFrame: " << pointsPerFrame << "\n";
      analogPerFrame = 0;
      std::cout << "analogPerFrame: " << analogPerFrame << "\n";
      iPoints=NULL;
      iAnalog=NULL;
      fPoints = new FPoint[pointsPerFrame*frames];
      fAnalog = new float32[analogPerFrame*frames];
      string x, y, z, c;
      float x_f, y_f, z_f, c_f;
      for (int i = 0; i < frames; i++){
              for (int j = 0; j < pointsPerFrame - 1; j++){
                  getline(ip,x,',');
                  getline(ip,y,',');
                  getline(ip,z,',');
                  getline(ip,c,',');
     // std::cout << i <<","<<j<< ", x: " << x <<","<<y<< "\n";
                  fPoints[i*pointsPerFrame + j].point[0] = std::stof(x) * 1000;
                  fPoints[i*pointsPerFrame + j].point[2] = std::stof(y) * 1000;
                  fPoints[i*pointsPerFrame + j].point[1] = std::stof(z) * 1000;
                  fPoints[i*pointsPerFrame + j].point[3] = std::stof(c) * 1000;
              }
              getline(ip,x,',');
              getline(ip,y,',');
              getline(ip,z,',');
              getline(ip,c,'\n');
    fPoints[i*pointsPerFrame + pointsPerFrame - 1].point[0] = std::stof(x);
    fPoints[i*pointsPerFrame + pointsPerFrame - 1].point[1] = std::stof(y);
    fPoints[i*pointsPerFrame + pointsPerFrame - 1].point[2] = std::stof(z);
    fPoints[i*pointsPerFrame + pointsPerFrame - 1].point[3] = std::stof(c);
          }

     ip.close();
}

void C3dData::countFrames(const char* filename){

    std::ifstream ip_temp(filename);

    string line;
    frames = 0;
    while(getline(ip_temp, line)){
        frames += 1;
    }
    ip_temp.close();
}

bool C3dFile::loadFile_csv(const char* filename)
{
   data.countFrames(filename);
   data.loadData_csv(filename);
   return true;
}

bool C3dFile::loadFile(const char* filename){
    string str = filename;

    string endwith = str.substr(str.size()-4);
    if (endwith.compare(".c3d") == 0){
        return loadFile_c3d(filename);
    }else if (endwith.compare(".csv") == 0){
        return loadFile_csv(filename);
    }else{
        std::cout << "[c3ddata] The current system do not support this file type. \nPlease adding a corresponding loadFile function in c3ddata.cpp file" << std::endl;
        return false;
    }
