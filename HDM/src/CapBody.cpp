...
dReal CapBody::density_HOG = 2100.0;

dReal CapBody::joint_cfm = 1e-8;
...
void CapBody::reloadBodySetting(){
    for (int ii=0;ii<JOINT_COUNT;++ii) {
      joint_target[ii][0]=0;
      joint_target[ii][1]=0;
      joint_target[ii][2]=0;
...
void CapBody::fillPositionFrame(DataFrame* bp)
{
  for (int ii=0;ii<BODY_COUNT;++ii) {

      dBodyID bodyid = body_segments[ii];
      const dReal* pos = dBodyGetPosition(bodyid);

    for (int ax=0;ax<3;++ax) {
      bp->setData(ii,ax,pos[ax]);
...
void CapBody::setTargetFrame2(DataFrame* jf, DataFrame* bp)
{
  if (!jf) return;
  if (!bp) return;
...
void CapBody::applyForceFrame2(DataFrame* jf, DataFrame* bp)
{
  if (!jf) return;
  if (!bp) return;
...
void CapBody::createBody(dWorldID world,dSpaceID space)
{
  ...
  createCaps(  UP_TORSO_BODY,UP_TORSO_GEOM,.14,.20, density);
  dBodySetQuaternion(body_segments[UP_TORSO_BODY],turnLeft);
  
  createCaps(  R_COLLAR_BODY,R_COLLAR_GEOM,.06,.09, density);
  createCaps(  RUP_ARM_BODY, RUP_ARM_GEOM, .05,.27, density_HOG);
  createCaps(  RLO_ARM_BODY, RLO_ARM_GEOM, .04 ,.17, density_HOG);
  createSphere(R_HAND_BODY,  R_HAND_GEOM,  .045);
  dBodySetQuaternion(body_segments[R_COLLAR_BODY],turnLeft);
  
  createCaps(  L_COLLAR_BODY,L_COLLAR_GEOM,.06,.09, density);
  createCaps(  LUP_ARM_BODY, LUP_ARM_GEOM, .05,.27, density_HOG);
  createCaps(  LLO_ARM_BODY, LLO_ARM_GEOM, .04 ,.17, density_HOG);
  ...
  createCaps(  WAIST_BODY,   WAIST_GEOM,   .07,.20, density);
  dBodySetQuaternion(body_segments[WAIST_BODY],turnLeft);
  
  createCaps(  RUP_LEG_BODY, RUP_LEG_GEOM, .10,.30, density);
  createCaps(  RLO_LEG_BODY, RLO_LEG_GEOM, .08,.28, density);
  createSphere(R_HEEL_BODY,  R_HEEL_GEOM,  .07);
  createCaps(  R_TARSAL_BODY,   R_TARSAL_GEOM,   .03,.04, density);
  dBodySetQuaternion(body_segments[R_TARSAL_BODY],turnLeft);
  
  createCaps(  LUP_LEG_BODY, LUP_LEG_GEOM, .10,.30, density);
  createCaps(  LLO_LEG_BODY, LLO_LEG_GEOM, .08,.28, density);
  createSphere(L_HEEL_BODY,  L_HEEL_GEOM,  .07);
  createCaps(  L_TARSAL_BODY,   L_TARSAL_GEOM,   .03,.04, density);
...
//    -M_PI/8,M_PI/8);
  ...
  dJointSetLMotorParam(joints[ROOT_LINMOTOR_JOINT],dParamCFM1,joint_cfm);
  dJointSetLMotorParam(joints[ROOT_LINMOTOR_JOINT],dParamCFM2,joint_cfm);
  dJointSetLMotorParam(joints[ROOT_LINMOTOR_JOINT],dParamCFM3,joint_cfm);
  ...
  dJointSetAMotorParam(joints[ROOT_ANGMOTOR_JOINT],dParamCFM1,joint_cfm);
  dJointSetAMotorParam(joints[ROOT_ANGMOTOR_JOINT],dParamCFM2,joint_cfm);
  dJointSetAMotorParam(joints[ROOT_ANGMOTOR_JOINT],dParamCFM3,joint_cfm);
...
void CapBody::createCaps(BodyType id,GeomType g,dReal radius,dReal zLen, dReal density1 = density)
{
  dMass m;
  
  body_segments[id] = dBodyCreate(world);
  geometries[g] = dCreateCapsule(space,radius,zLen);
  dMassSetCapsule(&m,density1,3,radius,zLen);
  dBodySetMass(body_segments[id],&m);
  dGeomSetBody(geometries[g],body_segments[id]);
}
                         ...
                         dReal lo3,dReal hi3)
{
  ...
  dJointSetAMotorParam(motors[jj],dParamCFM1,joint_cfm);
  dJointSetAMotorParam(motors[jj],dParamCFM2,joint_cfm);
  dJointSetAMotorParam(motors[jj],dParamCFM3,joint_cfm);
...
void CapBody::findMassProperties()
{
  ...
  for (int ii=0;ii<BODY_COUNT;++ii) {
   dMass aa;
   dBodyGetMass(body_segments[ii], &aa);
   sumMM += aa.mass;
   dMassAdd(&sumM,&aa);
  }

  //std::cout << "mass: " << sumM.mass << "," << sumMM << std::endl;
 ...
 */
void CapBody::loadBody(const char* markerMap, const char* markerRelPos,const char* bodyDim, const char* jointAnchor)
{
  std::cout << "[CapBody] Default body setting files loaded..." << std::endl;
  loadMarkToBodyMap(markerMap);
  loadMarkRelPosMap(markerRelPos);
  loadBodyProperties(bodyDim);
  loadJointAnchors(jointAnchor);
}

void CapBody::loadCBody(const char* markerMap, const char* markerRelPos,const char* bodyDim, const char* jointAnchor)
{
  std::cout << "[CapBody] Body setting files loaded..." << std::endl;
  loadMarkToBodyMap(markerMap);
  loadMarkRelPosMap(markerRelPos);
  loadBodyProperties(bodyDim);
  loadJointAnchors(jointAnchor);
...
//  std::cout << jj << "," << ax << std::endl;
  if (dJointGetType(joints[jj]) ==  dJointTypeLMotor) {

//      std::cout << "Lmotor: " << dJointGetType(joints[jj]) << std::endl;
//      std::cout << "t1: " << feedback[jj].t1[0] << "," << feedback[jj].t1[1] << "," << feedback[jj].t1[2] << std::endl;
//      std::cout << "t2: " << feedback[jj].t2[0] << "," << feedback[jj].t2[1] << "," << feedback[jj].t2[2] << std::endl;
//      std::cout << "f2: " << feedback[jj].f2[0] << ", "<< feedback[jj].f2[1] << ", " << feedback[jj].f2[2] << std::endl;
//      std::cout << "gf: " << feedback[jj].gf[0] << "," << feedback[jj].gf[1] << "," << feedback[jj].gf[2] << "," ;
//      std::cout << feedback[jj].gf[3] << std::endl;
//      std::cout << "gf: " << feedback[jj].gf[4] << "," << feedback[jj].gf[5] << "," << feedback[jj].gf[6] << "," ;
//      std::cout << feedback[jj].gf[7] << std::endl;

  }else if (dJointGetType(joints[jj]) ==  dJointTypeAMotor){

      // std::cout << "Amotor: " << dJointGetType(joints[jj]) << std::endl;

  }else{

    //  std::cout << "other motors: " << dJointGetType(joints[jj]) << std::endl;

  }

//  switch (dJointGetType(joints[jj])) {
//  case dJointTypeLMotor:
//    std::cout << "linear: " << feedback[jj].f1[ax] << std::endl;
//    break;
//  default:
//    val = feedback[jj].t1[ax];
//    std::cout << "angular: " << feedback[jj].f1[ax] << std::endl;
//    break;
//  }


 // std::cout << "global_forces:" << global_forces << std::endl;
...
double CapBody::forceValue2(int jj,int ax)
{
  double val;

  if (dJointGetType(joints[jj]) ==  dJointTypeLMotor) {

    val = val=feedback[jj].gf[ax];

  }else if (dJointGetType(joints[jj]) ==  dJointTypeAMotor){

      // std::cout << "Amotor: " << dJointGetType(joints[jj]) << std::endl;
      val = val=feedback[jj].t1[ax];
...
void CapBody::updateBodyMarkMapCMU(vector<string> labels){
    marker_to_body.resize(labels.size());
    for (int ii = 0; ii < labels.size(); ii++) {
       marker_to_body[ii].id = bodyMarkMap[labels[ii]];
    }
    copyToSim();
    std::cout << "[CapBody] Update body-marker relative positions."<< std::endl;
}

void CapBody::updatePosMapCMU(vector<string> labels){
    marker_to_body.resize(labels.size());
    for (int ii = 0; ii < labels.size(); ii++) {
        for (int jj = 0; jj<3; jj++){
            marker_to_body[ii].position[jj] = posMap[labels[ii]][jj];
        }
    }
    copyToSim();
    std::cout << "[CapBody] Update relationships between markers and body segments ."<< std::endl;
}
