    ...
    QGLWidget(parent)
{
  ...
  defaultView();
  std::cout << "[myglwidget] SimWorld object create..." << std::endl;
...
bool MyGLWidget::eventFilter(QObject *obj, QEvent *event) {
    if(event->type() == QEvent::KeyPress) {
        QKeyEvent *keyEvent = static_cast<QKeyEvent *>(event);
          ...
          if (keyEvent->modifiers()&Qt::ControlModifier|true) {
...
//                   vector<double>  dxyz = computeDeltaXYZ(trans);
//                   camera.translateAbsolute(QVector3D( dxyz[0],dxyz[1],dxyz[2]));
...
void MyGLWidget::defaultView()
{
    camera.reset();
    camera.translateRelative(QVector3D(0,-6,2));
    camera.rotateRelative(QQuaternion::fromAxisAndAngle(0,1,0,90));
    camera.rotateRelative(QQuaternion::fromAxisAndAngle(0,0,1,90));
    camera.rotateRelative(QQuaternion::fromAxisAndAngle(0,1,0,-90));
...
double MyGLWidget::computeDeltaDistance(double deltaAngle)
{

    dBodyID waist = sim_world->getBody()->body_segments[CapBody::WAIST_BODY];
    const dReal* pos = dBodyGetPosition(waist);
    QVector3D cPos = camera.getPosition();

    double distacne = sqrt(pow(cPos.x()-pos[0], 2) +
                          pow(cPos.y()-pos[1], 2) +
                          pow(cPos.z()-pos[1], 2) * 1.0);
    return tan(deltaAngle * 3.14159265 / 180);
}


vector<double> MyGLWidget::computeDeltaXYZ(double deltaAngle)
{

//    dBodyID waist = sim_world->getBody()->body_segments[CapBody::WAIST_BODY];
//    const dReal* pos = dBodyGetPosition(waist);
//    QVector3D cPos = camera.getPosition();
//    double vx = pos[0] - cPos.x();
//    double vy = pos[1] - cPos.y();
//    double vz = pos[2] - cPos.z();
//    double distacne = sqrt(vx * vx + vy * vy + vz * vz);
//    vector<double> dxyz;
//    dxyz.push_back(vx / distacne);
//    dxyz.push_back(vy / distacne);
//    dxyz.push_back(vz / distacne);
//    return dxyz;
}
...
void MyGLWidget::initializeGL()
{
  ...
  QImage img(QString("data/icons/seamlessNoise.png"));
  noise_texture = bindTexture(img,GL_TEXTURE_2D,GL_RGB,QGLContext::DefaultBindOption);
  racquet_texture = bindTexture(QImage(QString("data/icons/racquetTex.png")),GL_TEXTURE_2D,GL_RGBA,QGLContext::DefaultBindOption);
  wood_texture = bindTexture(QImage(QString("data/icons/wood.bmp")),GL_TEXTURE_2D,GL_RGB,QGLContext::DefaultBindOption);
  cement_texture = bindTexture(QImage(QString("data/icons/concrete.bmp")),GL_TEXTURE_2D,GL_RGB,QGLContext::DefaultBindOption);
  ceiling_texture = bindTexture(QImage(QString("data/icons/ceiling.bmp")),GL_TEXTURE_2D,GL_RGB,QGLContext::DefaultBindOption);
...
void MyGLWidget::renderSimWorld(SimWorld* world)
{
  ...
  if (world->usingDefaultSettings == false) {
      if (world->drawObjects != "") {
        renderCustomSettings(world->drawObjects.c_str(), world->objectFrames);
      }
  }
    ...
    for (int ii=0;ii<mCount;++ii) {
      ...
      if (id>=0 && id<=mCount) {
        dBodyGetRelPointPos(cb->body_segments[id],
                          rPos[0],rPos[1],rPos[2],mappedPoint); // check: mapped point[0] == marker[0] position
        if (mPos[2]>0) {
...
//////////////////////////////////////////////////////
/////////////////////Custom Code//////////////////////
void MyGLWidget::renderCustomSettings(const char* filename, int frames){

        std::ifstream ip(filename);

        if(!ip.is_open()) {
            std::cout << "[myglwidget] ERROR: File Open" << '\n';
            return;
        }

        float curve_position [frames][6];
        string x, y, z, a, b, c;
        for (int i = 0; i < frames; i++){
                    ...
                    curve_position[i][0] = std::stof(x);
                    curve_position[i][2] = std::stof(y);
                    curve_position[i][1] = std::stof(z);
                    curve_position[i][3] = std::stof(a);
                    curve_position[i][5] = std::stof(b);
                    curve_position[i][4] = std::stof(c);
            }
        ...
        for (int i=0;i<frames;++i) {
            glColor3d(0,.2,.5);
            glPushMatrix();
            glTranslated(curve_position[i][0],curve_position[i][1],curve_position[i][2]);
            gluSphere(quadric,.01,2,3);
            glPopMatrix();
        }
}


void MyGLWidget::customCamera(){

    camera.rotateRelative(QQuaternion:: fromAxisAndAngle(0,1,0,90));
    camera.translateRelative(QVector3D(6,-1,5));
}
